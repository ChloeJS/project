...
<script>
   ...
   // 이메일 형식 체크(복붙)
      ...
      $("#dupCheckBtn").on("click", )
      var isCon = true;  // 진행이 잘 되고있는지 여부를 확인하기 위한 변수 설정 (변수의 스코프)
      $.ajax({
           	 url : 'emailDupCheck.do',
             data : {email : $("#email").val()},
             async : false, // 순차적으로 실행되도록 하려면 false로 해야함?
             success : function(res) { // true나 false가 들어있음
                if (res == 'true') {
                   alert('중복된 이메일입니다. 다른 이메일을 입력해주세요.');
                   $("#email").val("");
                   $("#email").focus();
                   isCon= false; // 여기서 바꾸면 중지시킬 수 있음
                   //return; 여기서 return은 ajax의 리턴이므로 전체 함수의 리턴을 의미하는것이아니다.
                   //중지가 안된 이유는 콜백함수라서? 비동기가 아니라 동기로 하면?
                } else{
                   alert("사용가능한 이메일");
                }
             }
      });
      if(!isCon) return; // 중지시키게끔 조건을 일치시켜줌
      if($("#pwd").val().trim()==''){ // 비밀번호 입력 확인
         alert("비밀번호를 입력해주세요"); // ajax라서 먼저 뜬다?
         $("#pwd").focus(); // 커서
         return;
      }
      ...
      if(!reg.test($("#pwd").val())){
         alert("비밀번호는 숫자/대문자/소문자/특수문자 조합으로 8자이상 입력하세요!");
         return; // alert띄우고 return으로 중지
      }
      
      if($("#pwd").val() !=$("#pwd_check").val()){ // 사용자가 입력한 비밀번호와 비밀번호 확인값이 다르면!
         alert("비밀번호를 확인해주세요!!!");
         $("#pwd_check").focus();
         return;
      }
  	if ($("#name").val().trim() == '') { // 이름 입력 확인
		alert('이름을 입력해 주세요.');
		$("#name").focus();
		return;
	  }
...
<!-- 우편번호 API 넣는 방법 (함수이름 zipcode로 변경)-->
        ...
        <div class="sub">
            <div class="size">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>
                                ...
                                <span class="email_check"><a href="javascript:;" id="dupCheckBtn" class="btn bgGray" style="float:left; width:auto; clear:none;">중복확인</a></span>
                            </td>
                        </tr>
                        <tr>
                            <th>*비밀번호</th>
                ...
                <div class="btnSet clear">
                    <div><a href="javascript:;" class="btn" onclick="goSave();">가입</a> 
